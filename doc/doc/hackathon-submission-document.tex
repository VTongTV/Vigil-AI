...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \fill[vigilcyan!35] ([xshift=-2cm,yshift=2cm]current page.north east) circle (5.4cm);
  \fill[vigilcyan!25] ([xshift=1.4cm,yshift=-1.4cm]current page.south west) circle (5.1cm);
...
\vspace{0.2cm}
{\small\color{white!80}\url{https://github.com/VTongTV/Vigil-AI.git}}

\vspace{0.5cm}
\fcolorbox{vigilgreen}{deepnavy}{\begin{minipage}{0.92\linewidth}\centering
\vspace{6pt}
{\Large\bfseries\color{vigilgreen} LIVE \& DEPLOYED}\\[6pt]
{\normalsize\color{white}This system is running right now. Anyone can try it without any setup.}\\[8pt]
{\large\bfseries\color{vigilcyan} Frontend Dashboard:}\;\;{\normalsize\color{white}\url{https://vigil-ai-gules.vercel.app/dashboard}}\\[6pt]
{\large\bfseries\color{vigilcyan} Backend API Docs:}\;\;{\normalsize\color{white}\url{https://vtongtv-vigilai-backend.hf.space/docs}}\\[6pt]
{\small\color{white!80}HuggingFace Spaces (CPU Basic, 16\,GB RAM) + Vercel (CDN) \;\textbullet\; Zero cost, zero GPU required}\\[4pt]
\vspace{6pt}
\end{minipage}}
...
\hline
\textbf{What is required} & \textbf{What VigilAI demonstrates} \\
\hline
...
\noindent\textbf{Why our approach is better:} it is not only an object detector. It includes the full enforcement workflow: image or video intake $\rightarrow$ AI detection $\rightarrow$ OCR $\rightarrow$ evidence generation $\rightarrow$ officer approval / rejection $\rightarrow$ analytics and reporting.

\vspace{8pt}

\judgecallout{Novel areas}{VigilAI turns traffic images into officer-verified, tamper-evident enforcement records using practical computer vision and a police-oriented command center.}

\vspace{8pt}

\fcolorbox{vigilgreen}{deepnavy}{\begin{minipage}{0.94\linewidth}
\vspace{4pt}
{\bfseries\large\color{vigilgreen} LIVE DEMO — No setup required}\\[6pt]
{\bfseries\color{vigilcyan}Frontend:}\;\;{\color{white}\url{https://vigil-ai-gules.vercel.app/dashboard}}\\[4pt]
{\bfseries\color{vigilcyan}Backend API:}\;\;{\color{white}\url{https://vtongtv-vigilai-backend.hf.space/docs}}\\[4pt]
{\small\color{white!80}Deployed on HuggingFace Spaces (Docker, CPU Basic, 16\,GB RAM) + Vercel (CDN). Both free tier. Judges: open the dashboard link and try it.}
\vspace{4pt}
\end{minipage}}
...
This section lists the current prototype capabilities plainly, without relying on inflated claims. The aim is to make every shipped feature easy for officers to find.
...
VigilAI is not a local prototype. It is deployed and accessible right now on the open web, running on cloud infrastructure at zero cost.

\begin{center}
\fcolorbox{vigilgreen}{deepnavy}{\begin{minipage}{0.94\linewidth}
\vspace{6pt}
\begin{center}{\Large\bfseries\color{vigilgreen} LIVE \& DEPLOYED}\end{center}
\vspace{6pt}
\begin{tabular}{@{}p{3.2cm}p{3cm}p{6cm}@{}}
\hline
{\bfseries\color{white}Component} & {\bfseries\color{white}Platform} & {\bfseries\color{white}URL} \\
\hline
{\color{white}Frontend Dashboard} & {\color{white}Vercel (CDN)} & {\color{vigilcyan}\url{https://vigil-ai-gules.vercel.app/dashboard}} \\
{\color{white}Backend API Docs} & {\color{white}HuggingFace Spaces} & {\color{vigilcyan}\url{https://vtongtv-vigilai-backend.hf.space/docs}} \\
{\color{white}Source Code} & {\color{white}GitHub} & {\color{vigilcyan}\url{https://github.com/VTongTV/Vigil-AI}} \\
\hline
\end{tabular}
\vspace{6pt}
\end{minipage}}
...
\begin{itemize}
  \item \textbf{Zero setup.} No cloning, no \texttt{pip install}, no GPU required. Open the dashboard URL and start testing.
  \item \textbf{Always available.} The Vercel frontend never sleeps. The HuggingFace Spaces backend auto-wakes on request with cold-boot retry handled client-side.
  \item \textbf{Real infrastructure.} Docker-based backend on HuggingFace Spaces CPU Basic (16\,GB RAM). Not a mock. Not a notebook. A production-style deployment.
  \item \textbf{Zero cost.} Both platforms are free tier. No credit card, no trial period, no expiry before the hackathon deadline.
\end{itemize}

\subsection{Cloud Deployment Stack}

\begin{longtable}{@{}p{3.6cm}p{4cm}p{5.5cm}@{}}
\hline
...
% ============================================================
\section{Suggested Demo Flow}
\sectionrule

\judgecallout{Quick start}{Open \textbf{\url{https://vigil-ai-gules.vercel.app/dashboard}} in any browser. No setup, no installation, no GPU. The system is live.}
